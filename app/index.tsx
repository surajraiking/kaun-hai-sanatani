...
import { askMuniSalah, generateOnlineQuestions, questionFingerprint } from '@/lib/gemini';
...
function pickRoundQuestions(saved: SavedProgress) {
  ...
  const pool = [...saved.generatedQuestions, ...QUESTION_BANK];
  const unique = new Map<string, Question>();
  for (const q of pool) {
    const fp = questionFingerprint(q);
    if (!usedIds.has(q.id) && !usedFingerprints.has(fp)) unique.set(q.id, q);
  }
      ...
      if (isDaily) {
        ...
        if (selectedQuestions.length < TOTAL_PADAS) {
          const available = [...saved.generatedQuestions, ...QUESTION_BANK]
            ...
            .filter((q) => !saved.usedQuestionIds.includes(q.id) && !saved.usedQuestionFingerprints.includes(questionFingerprint(q)))
            .sort(() => Math.random() - 0.5);
